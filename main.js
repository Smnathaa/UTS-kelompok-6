...
function tambahSapi() {
  ...
  if (!nama || isNaN(umur) || isNaN(berat)) {
    alert("Lengkapi data sapi!");
    return;
  }

  sapiList.push({ nama, umur, berat });
  renderSapi();
...
function beriMakan(index) {
  ...
  alert(sapiList[index].nama + " sudah diberi makan, berat bertambah!");
  renderSapi();
}

function jualSapi(index) {
  alert(sapiList[index].nama + " sudah dijual!");
